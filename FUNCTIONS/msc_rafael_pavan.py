...
def fluxo_de_pot(grupo, sep):
    ...
    for linha in range(grupo.shape[0]):
        ...
        if len(sep.bus)==300:
            ...
            pp.runpp(sep,algorithm='nr',numba=True, init = 'results', tolerance_mva = 1e-7,max_iteration=10000,enforce_q_lims=False)
...
def otimizacao_pso_discreto(sep, zeta, psi, sigma, omega, max_iter, n_particles,c1,c2,v_amp,valor_inicial,relatorio=True,inicial=True):
...
def ipm_ieee30 (sep30, solver, rtol, otol, max_iter, relatorio = True, remoto = True, arredondado = True, travado = True):
    ...
    tap_max = np.ones(len(tap))*1.1


    tap_min = np.ones(len(tap))*0.9
    ...
    if travado == True:
        ...
        sh9 = sep.Var(value = sh[9],lb = -0.39, ub = 0)                        
...
def ipm_ieee14 (sep14, solver, rtol, otol, max_iter, relatorio = True, remoto = True, arredondado = True, travado = True):
