...
def fluxo_de_pot(grupo, sep):
    ...
    for linha in range(grupo.shape[0]):
        ...
        if len(sep.bus)==300:
            ...
            pp.runpp(sep,algorithm='fdbx',numba=True, init = 'results', tolerance_mva = 1e-10,max_iteration=1000,enforce_q_lims=False,trafo_model='pi')
...
def otimizacao_pso_discreto(sep, zeta, psi, sigma, omega, max_iter, n_particles,c1,c2,v_amp,valor_inicial,step, wmax,relatorio=True,inicial=True):
    ...
    for i in range(0,max_iter):
        ...
        v_novo[:,n_vgen+n_tap:n_vgen+n_tap+n_bshunt][v_novo[:,n_vgen+n_tap:n_vgen+n_tap+n_bshunt]>1*0.01] = 1*0.01
...
def otimizacao_clpso_discreto(sep, pt, rgap, zeta, psi, sigma, omega, max_iter, n_particles,c1,c2,v_amp,valor_inicial,step, wmax,relatorio=True,inicial=True):
    ...
    for i in range(0,max_iter):
        ...
        global_best[n_vgen:n_vgen+n_tap] = discreto_tap(global_best.copy(),n_tap,n_vgen,n_bshunt,sep)
        
        global_best[n_vgen+n_tap:n_vgen+n_tap+n_bshunt] = discreto_bshunt(global_best.copy(),n_tap,n_vgen,n_bshunt,sep)
        ...
        v_novo[:,n_vgen+n_tap:n_vgen+n_tap+n_bshunt][v_novo[:,n_vgen+n_tap:n_vgen+n_tap+n_bshunt]>1*step] = 1*step
...
#         custo
    ...
    for linha in range(grupo.shape[0]):
        ...
        if len(sep.bus)==300:
            ...
            pp.runpp(sep,algorithm='fdbx', numba=True, init = 'results', tolerance_mva = 1e-7,max_iteration=1000,enforce_q_lims=False,trafo_model='pi')
...
def fluxo_de_pot_fpo_q(grupo, sep):
    ...
    for linha in range(grupo.shape[0]):
        ...
        if len(sep.bus)==300:
            ...
            pp.runpp(sep,algorithm='fdbx',numba=True, init = 'flat', tolerance_mva = 1e-6,max_iteration=10000,trafo_model='pi')
        
        else:
        
            pp.runpp(sep,algorithm='nr',numba=True, init = 'results', tolerance_mva = 1e-6,max_iteration=1000,enforce_q_lims=True,trafo_model='pi')
