...
def Comprehensive_Learning_PSO(sep, refresh_gap, p1_, zeta, psi, sigma, omega, neta, max_iter, n_particles,c1, v_amp,valor_inicial,step, wmax,wmin,relatorio=True,inicial=True):
    ...
    for i in range(0,max_iter):
        ...
        for li in range(n_particles):
            ...
            if refresh_rate[li] >= refresh_gap:
                ...
                for p in range(len(pbestfi[li,:])-7):

                    if pbestfi[li,p] == best_particles[li,p]:

                        flag = flag+1

                if flag == len(pbestfi[li,:])-7:
        ...
        v_novo = np.multiply(w_novo,v_anterior.copy()) + c1*np.multiply(r1,(pbestfi.copy()-enxame_fit.copy())) + 0.5*np.multiply(r2,(global_best.copy()-enxame_fit.copy())) 
